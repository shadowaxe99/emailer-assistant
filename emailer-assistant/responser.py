...
def generate_resp(history, sender_email, owner_email, free_time):
    ...
    context = """You are a virtual assistant acting as a bridge between two individuals for scheduling meetings.
               ...
               explicitly, try to arrange 2-3 suitable hours from the available time.""".format(
        str(today), owner_email, free_time, sender_email
    )
    ...
    messages_list = [query("system", context), query("user", prompt)]

    # Generate a response using GPT-4
    print("GENERATING RESPONSE")
    response = openai.ChatCompletion.create(model="gpt-4", messages=messages_list)
    print("GOT IT")
